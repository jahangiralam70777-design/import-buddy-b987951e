...
export const adminListUsers = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // Look up email + verification status from auth.users (parallel).
      ...
      ids.map(async (id) => {
        try {
          ...
          emailMap.set(id, {
            ...
            verified: !!u?.user?.email_confirmed_at,
          });
        } catch {
          emailMap.set(id, { email: null, verified: false });
        }
      }),
    );
    ...
    let rows = (profiles ?? []).map((p: { id: string }) => ({
      ...
      email_verified: emailMap.get(p.id)?.verified ?? false,
    }));
