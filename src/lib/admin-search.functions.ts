...
export const adminGlobalSearch = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<AdminGlobalSearchResult> => {
    ...
    for (const u of (users.data ?? []) as any[]) {
      hits.push({ kind: "user", id: u.id, label: u.display_name ?? "Unnamed user", sub: u.level ?? null, to: `/admin/users?focus=${u.id}` });
    }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    for (const s of (subjects.data ?? []) as any[]) {
      hits.push({ kind: "subject", id: s.id, label: s.name, sub: s.level ?? null, to: `/admin/academic-manager?subject=${s.id}` });
    }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    for (const c of (chapters.data ?? []) as any[]) {
      hits.push({ kind: "chapter", id: c.id, label: c.name, sub: "Chapter", to: `/admin/academic-manager?chapter=${c.id}` });
    }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    for (const m of (mcqs.data ?? []) as any[]) {
      hits.push({ kind: "mcq", id: m.id, label: (m.question as string).slice(0, 80), sub: "MCQ", to: `/admin/mcq?focus=${m.id}` });
    }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    for (const q1 of (quizzes.data ?? []) as any[]) {
      hits.push({ kind: "quiz", id: q1.id, label: q1.title, sub: "Quiz", to: `/admin/quiz?focus=${q1.id}` });
    }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    for (const mk of (mocks.data ?? []) as any[]) {
      hits.push({ kind: "mock", id: mk.id, label: mk.title, sub: "Mock Test", to: `/admin/mock-test?focus=${mk.id}` });
    }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    for (const n of (notes.data ?? []) as any[]) {
      hits.push({ kind: "note", id: n.id, label: n.title, sub: n.level ?? "Note", to: `/admin/short-notes?focus=${n.id}` });
    }

    const counts = hits.reduce(
      (acc, h) => ({ ...acc, [h.kind]: (acc[h.kind] ?? 0) + 1 }),
      { user: 0, subject: 0, chapter: 0, mcq: 0, quiz: 0, mock: 0, note: 0 } as AdminGlobalSearchResult["counts"],
    );
