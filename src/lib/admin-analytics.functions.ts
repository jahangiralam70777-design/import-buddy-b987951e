...
export const adminAnalyticsOverview = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [
      ...
      sb.from("exam_attempts").select("id,correct_count,total_count,duration_seconds,user_id,quiz_id,kind,subject_id,chapter_id,level,created_at,title").eq("status", "completed").gte("created_at", fromIso).lte("created_at", toIso).limit(5000),
      ...
      sb.from("exam_attempts").select("user_id,created_at").gte("created_at", todayStart.toISOString()).limit(5000),
      sb.from("exam_attempts").select("user_id").gte("created_at", sevenAgo.toISOString()).limit(10000),
      sb.from("exam_attempts").select("user_id").gte("created_at", prev7Start.toISOString()).lt("created_at", sevenAgo.toISOString()).limit(10000),
      sb.from("exam_attempts").select("user_id").gte("created_at", thirtyAgo.toISOString()).limit(20000),
      sb.from("exam_attempts").select("user_id").gte("created_at", prev30Start.toISOString()).lt("created_at", thirtyAgo.toISOString()).limit(20000),
      sb.from("exam_attempts").select("id,user_id,kind,score,correct_count,total_count,created_at,title,status").order("created_at", { ascending: false }).limit(15),
      sb.from("mcq_bookmarks").select("id", { count: "exact", head: true }),
      sb.from("mcq_wrong_questions").select("id", { count: "exact", head: true }),
    ]);

    const completed = (attemptsCompleted.data ?? []) as Array<{ correct_count: number; total_count: number; duration_seconds: number; user_id: string; quiz_id: string; kind: string; subject_id: string | null; chapter_id: string | null; level: string | null; created_at: string; title: string | null }>;
    ...
    // DAU/WAU/MAU
    const dau = new Set(((dauAttempts.data ?? []) as Array<{ user_id: string }>).map((a) => a.user_id)).size;
    const wau = new Set(((wau7.data ?? []) as Array<{ user_id: string }>).map((a) => a.user_id)).size;
    const wauPrev = new Set(((wauPrev7.data ?? []) as Array<{ user_id: string }>).map((a) => a.user_id)).size;
    const mau = new Set(((mau30.data ?? []) as Array<{ user_id: string }>).map((a) => a.user_id)).size;
    const mauPrev = new Set(((mauPrev30.data ?? []) as Array<{ user_id: string }>).map((a) => a.user_id)).size;
    const wauGrowth = wauPrev > 0 ? Math.round(((wau - wauPrev) / wauPrev) * 1000) / 10 : (wau > 0 ? 100 : 0);
    const mauGrowth = mauPrev > 0 ? Math.round(((mau - mauPrev) / mauPrev) * 1000) / 10 : (mau > 0 ? 100 : 0);
    ...
    const levelStatsMap = new Map<string, { users: number; attempts: number; correct: number; total: number }>();
    for (const [k, v] of levelProfileCounts) levelStatsMap.set(k, { users: v, attempts: 0, correct: 0, total: 0 });
    ...
    const levelStats = Array.from(levelStatsMap.entries()).map(([level, s]) => ({
      level,
      users: s.users,
      attempts: s.attempts,
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      months.push({ label: d.toLocaleString("en", { month: "short" }), key: `${d.getFullYear()}-${d.getMonth()}`, start: d, end: next });
    }
    ...
    for (const q of (quizzes ?? []) as Array<{ id: string; subject_id: string | null; chapter_id: string | null }>) {
    ...
    const chapterEngagement = Array.from(chapStats.entries())
      .map(([cid, s]) => {
        ...
        const subj = ch ? (subjects ?? []).find((sb: { id: string }) => sb.id === ch.subject_id) : null;
        return {
          id: cid,
    ...
    const recent = (recentAttempts.data ?? []) as Array<{ id: string; user_id: string; kind: string; score: number; correct_count: number; total_count: number; created_at: string; title: string | null; status: string }>;
    ...
    const recentActivity = recent.map((r) => {
      ...
      return {
        ...
        level: p?.level ?? "—",
        kind: r.kind,
        title: r.title ?? (r.kind === "mock" ? "Mock test" : r.kind === "quiz" ? "Quiz attempt" : "Practice"),
    ...
    return {
      ...
      kpis: {
        ...
        contentItems: (mcqsCount.count ?? 0) + (notesCount.count ?? 0) + (videosCount.count ?? 0) + (flashCount.count ?? 0) + (quizCount.count ?? 0) + (mockCount.count ?? 0),
        dau,
        wau,
        mau,
        wauGrowth,
...
const rangeInput = z.object({
  rangeHours: z.number().int().min(1).max(24 * 365).optional(),
  limit: z.number().int().min(1).max(100).optional(),
  bucketMinutes: z.number().int().min(1).max(24 * 60).optional(),
});
...
export const adminActivityOverview = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: row, error } = await context.supabase.rpc("admin_activity_overview", { _range_hours: hours });
    if (error) throw error;
...
export const adminTopButtons = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (error) throw error;
    return (rows ?? []) as Array<{ element_id: string; element_label: string; page_path: string; click_count: number }>;
  });
...
export const adminUserActivity = createServerFn({ method: "POST" })
  ...
  .inputValidator((i: { userId: string; limit?: number }) =>
    z.object({ userId: z.string().uuid(), limit: z.number().int().min(1).max(200).optional() }).parse(i),
  )
  .handler(async ({ data, context }) => {
    await assertAdmin(context.supabase, context.userId);
    ...
    return (rows ?? []) as unknown as Array<{
      id: string; user_id: string; event_type: string; page_path: string | null;
      element_label: string | null; module: string | null; metadata: Json; created_at: string;
    }>;
  });
...
export const adminActivityFeed = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    let q = sb.from("activity_events")
      .select("id,user_id,event_type,page_path,element_id,element_label,module,target_kind,target_id,metadata,created_at")
      .gte("created_at", since)
      .order("created_at", { ascending: false })
      .limit(data.limit ?? 50);
    if (data.eventType) q = q.eq("event_type", data.eventType);
    if (data.userId) q = q.eq("user_id", data.userId);
    if (data.search) q = q.or(`element_label.ilike.%${data.search}%,page_path.ilike.%${data.search}%,element_id.ilike.%${data.search}%`);
    const { data: rows, error } = await q;
    if (error) throw error;

    const rowsAny = (rows ?? []) as unknown as Array<{
      id: string; user_id: string | null; event_type: string; page_path: string | null;
      element_id: string | null; element_label: string | null; module: string | null;
      target_kind: string | null; target_id: string | null; metadata: Json; created_at: string;
    }>;
    ...
    const pmap = new Map((profiles ?? []).map((p: { id: string; display_name: string | null; avatar_url: string | null }) => [p.id, p]));
