...
export const studentAdvancedAnalytics = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const weeklyGoal = 100; // MCQs answered per week
    ...
    // Goals progress
    const weeklyStudyGoalMin = 300; // 5h
    const monthlyStudyGoalMin = 1200; // 20h
    const goals = {
      weekly: {
        studyMinutes: Math.round(studyTime.week / 60),
        target: weeklyStudyGoalMin,
        percent: Math.min(100, Math.round((studyTime.week / 60 / weeklyStudyGoalMin) * 100)),
      },
      monthly: {
        studyMinutes: Math.round(studyTime.month / 60),
        target: monthlyStudyGoalMin,
        percent: Math.min(100, Math.round((studyTime.month / 60 / monthlyStudyGoalMin) * 100)),
      },
    };

    return {
      studyTime: {
        todaySeconds: studyTime.today,
        weekSeconds: studyTime.week,
        monthSeconds: studyTime.month,
        daily: studyDaily,
      },
      totals: {
