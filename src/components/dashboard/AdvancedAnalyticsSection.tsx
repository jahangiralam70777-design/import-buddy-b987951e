...
import {
  ArrowDownRight, ArrowUpRight, BookOpen, Brain, Calendar, CheckCircle2,
  Clock, Flame, Goal, Sparkles, Target, TrendingDown, TrendingUp, XCircle, Zap,
} from "lucide-react";
...
export function AdvancedAnalyticsSection() {
  ...
  const { studyTime, totals, byKind, subjectAccuracy, chapterAccuracy, strongTopics, weakTopics, heatmap, streak, insights, goals } = data;
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="grid grid-cols-2 gap-4 md:grid-cols-4">
        <PremiumCard
          title="Today's Study"
          value={fmtSeconds(studyTime.todaySeconds)}
          sub="Active focus time"
          icon={<Clock className="h-5 w-5" />}
          accent="var(--neon-purple)"
        />
        <PremiumCard
          title="This Week"
          value={fmtSeconds(studyTime.weekSeconds)}
        ...
        <PremiumCard
          title="This Month"
          value={fmtSeconds(studyTime.monthSeconds)}
          sub={`${goals.monthly.percent}% of monthly goal`}
      ...
      {/* === Study time chart + Accuracy donut + Streak === */}
      <section className="grid grid-cols-1 gap-5 lg:grid-cols-3">
        <div className="glass shadow-card-soft rounded-3xl p-5 lg:col-span-2">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-display text-lg font-bold">Daily Study Time</h3>
              <p className="text-xs text-muted-foreground">Minutes per day · last 7 days</p>
            </div>
            <div className="glass rounded-xl px-3 py-1.5 text-xs">{fmtSeconds(studyTime.weekSeconds)} total</div>
          </div>
          <div className="mt-4 h-56">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={studyTime.daily} margin={{ top: 8, right: 8, left: -16, bottom: 0 }}>
                <defs>
                ...
                />
                <Area type="monotone" dataKey="minutes" stroke="var(--neon-purple)" strokeWidth={2.5} fill="url(#studyAreaFill)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <section className="grid grid-cols-1 gap-5 md:grid-cols-3">
        <div className="glass shadow-card-soft rounded-3xl p-5">
          <div className="flex items-center justify-between">
            <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Weekly Goal</p>
            <Goal className="h-4 w-4 text-[var(--neon-blue)]" />
          </div>
          <p className="font-display mt-2 text-2xl font-bold">{goals.weekly.studyMinutes} <span className="text-sm text-muted-foreground">/ {goals.weekly.target} min</span></p>
          <div className="mt-3 h-2 overflow-hidden rounded-full bg-muted">
            <div className="h-full rounded-full bg-gradient-to-r from-[var(--neon-blue)] to-[var(--neon-purple)] transition-all duration-700"
              style={{ width: `${goals.weekly.percent}%` }} />
          </div>
          <p className="mt-2 text-xs text-muted-foreground">{goals.weekly.percent}% complete</p>
        </div>
        <div className="glass shadow-card-soft rounded-3xl p-5">
          <div className="flex items-center justify-between">
            <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Monthly Goal</p>
            <Goal className="h-4 w-4 text-[var(--neon-purple)]" />
          </div>
          <p className="font-display mt-2 text-2xl font-bold">{goals.monthly.studyMinutes} <span className="text-sm text-muted-foreground">/ {goals.monthly.target} min</span></p>
          <div className="mt-3 h-2 overflow-hidden rounded-full bg-muted">
            <div className="h-full rounded-full bg-gradient-to-r from-[var(--neon-purple)] to-[var(--neon-pink)] transition-all duration-700"
              style={{ width: `${goals.monthly.percent}%` }} />
          </div>
          <p className="mt-2 text-xs text-muted-foreground">{goals.monthly.percent}% complete</p>
        </div>
